...
class DobleClick extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `<div style="border:1px solid blue; padding:10px;">Haz doble click aquí</div>`;
        this.ondblclick = () => {
            alert("¡Hiciste doble click!");
        };
    }
}
customElements.define("doble-click", DobleClick);

class MouseOver extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `<div style="border:1px solid green; padding:10px;">Pasa el mouse por aquí</div>`;
        this.onmouseover = () => {
...
customElements.define("mouse-over", MouseOver);

class TecladoInput extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `<input type="text" placeholder="Escribe algo..."> <p id="texto"></p>`;
        let input = this.querySelector("input");
        let p = this.querySelector("#texto");
        input.addEventListener("keyup", () => {
            p.innerText = "Escribiste: " + input.value;
        });
    }
}
customElements.define("teclado-input", TecladoInput);